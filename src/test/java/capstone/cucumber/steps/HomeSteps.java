...
public class HomeSteps extends CommonUtility{
	...
	@Then("below options are present in Shop by Department sidebar")
	 ...
	 Assert.assertTrue(isElementDisplayed(getDriver().findElement(By.xpath("//span[text() = '" + 
	optionsToDisplay.get(0).get(0) + "']" ))));
	...
	String department;
	 ...
	 logger.info(SmartHomeLightning + " options are present in department" + 
	PlugsandOutlets);
	 break;
	 ...
	 String AutomativePartsAccessories = 
	getText(factory.homePage().AutomativePartsAccessories);
	 ...
	 logger.info(MotorCyclePowersports + " options are present in department" + 
	AutomativePartsAccessories);
	 break; 
	 } 
	 }
	 ...
	 String actualQ= factory.homePage().quantity.getText();
	
	 Assert.assertEquals(actualQ, two);
